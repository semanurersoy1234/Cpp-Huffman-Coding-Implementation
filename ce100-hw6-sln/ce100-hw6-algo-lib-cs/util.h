...
namespace shoc {
	...
	using byte = uint8_t;
	constexpr auto rol(uint32_t x, int s) { return x << s; }
	constexpr auto ror(uint32_t x, int s) { return x >> s; }
	constexpr bool little_endian() { return endian::native == endian::little; }
	 ...
	 * @param count Number of bytes
	 */
	void fill(void* dst, byte val, size_t count)
	{
	 ...
	 * @param count Number of bytes
	 */
	void zero(void* dst, size_t count)
	{
